...
synchronize——悲观锁
CyclicBarrier 的特点是要么大家都正常执行完毕，要么大家都异常被中断，不会其中有一个被中断而其它正常执行完毕的现象存在。
ReentrantLock、await由AQS实现的机制
+---------------+			  	+-----------------------+
|等待锁的线程队列……	| —进入另一个队列—>	|等待condition的线程队列……	|
+---------------+				+-----------------------+



二、并发工具
CountDownLatch —— 线程等待计数值为0方可继续进行，否则等待，不可循环使用
CyclicBarrier —— 一组线程互相等待，直到所有线程都到达屏障点（所有线程都调用了await()），这组线程才会越过屏障继续进行，可循环的
Semaphore —— 信号量，也就是说Semaphore不是可重入的。每一次请求一个许可都会导致计数器减少1，同样每次释放一个许可都会导致计数器增加1，一旦达到了0，新的许可请求线程将被挂起。
